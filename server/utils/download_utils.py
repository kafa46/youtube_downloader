...
def save_client_packet_info(client_request) -> bool:
    '''클라이언트 패킷 정보 저장'''
    ...
    client_dic = sanitize_json_or_dict(client_dic)
    device_dic = sanitize_json_or_dict(device_dic)
    ...
    try:
        db.session.commit()
        print('success commit to DB - visit data')
        return True
    except Exception as e:
        print(f'Error occurred - visit data: {e}')
        return False
...
def save_download_status(download_status: dict) -> bool:
    '''다운로드 상태 정보 저장'''
    dt = DownloadData()
    download_status = sanitize_json_or_dict(download_status)
    ...
    try:
        db.session.commit()
        print('success commit to DB - download data')
        return True
    except Exception as e:
        print(f'Error occurred - download data: {e}')
        return False
